...
import (
	...
	"omolsm/internal/lsm/sst_io/writer"
	"testing"
)
...
func writeSST(t *testing.T, conf *config.Config, kvs []KV) (size uint64, blockToFilter map[uint64][]byte, index []*sst_io.sst_io) {
	t.Helper()
...
func newTestConf(t *testing.T, blockSize int) *config.Config {
	...
	return &config.Config{
		...
		Filter:            filter2.NewBloomFilter(1024),
		FilterConstructor: func() filter2.Filter { return filter2.NewBloomFilter(1024) },
	}
}
