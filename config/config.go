...
import (
	"fmt"
	filter2 "omolsm/internal/lsm/filter"
	memtable2 "omolsm/internal/lsm/memtable"
	"os"
)
...
type Config struct {
	...
	Filter              filter2.Filter                // 供 Node 读路径 Exist 调用（只读，无状态）
	FilterConstructor   func() filter2.Filter         // 供 SSTWriter 创建独立实例（写路径，有状态）
	MemTableConstructor memtable2.MemTableConstructor // memtable 构造器，默认为跳表
}
...
func NewConfig(dir string, opts ...ConfigOption) (*Config, error) {
	c := Config{
		...
		SSTNumPerLevel:      10,
		SSTFooterSize:       32,
		Filter:              filter2.NewBloomFilter(10240), // 10240 bit ≈ 1.25KB
		FilterConstructor:   func() filter2.Filter { return filter2.NewBloomFilter(10240) },
		MemTableConstructor: memtable2.NewTreeMapMemTable,
	}
...
func WithFilter(f filter2.Filter) ConfigOption {
	return func(c *Config) {
		c.Filter = f
	}
}
...
func WithFilterConstructor(fc func() filter2.Filter) ConfigOption {
	return func(c *Config) {
...
func WithMemtableConstructor(memtableConstructor memtable2.MemTableConstructor) ConfigOption {
	return func(c *Config) {
